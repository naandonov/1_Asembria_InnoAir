...
final class NotificationsViewController: UIViewController {
    ...
    @objc func didLocateBeacons(_ notification: Notification) {
            let beacons = notification.userInfo?[TrackingManager.Constants.Notifications.UserInfoBeaconsKey] as? [Beacon]
        if let beacon = beacons?.first {
            fetchData(stopId: "\(beacon.major)")
        }
    ...
    private func fetchData(stopId: String) {
